...
class ChasmModule:
    ...
    def chasm(self):
        ...
        while True:
            ...
            if self.auto.find_element(
                "测评次数不足",
                "text",
                crop=(1141 / 2560, 684 / 1440, 1420 / 2560, 747 / 1440),
                is_log=self.is_log,
            ):
                break
            if self.auto.click_element(
                "确定",
                "text",
                crop=(1888 / 2560, 980 / 1440, 2020 / 2560, 1059 / 1440),
                is_log=self.is_log,
            ):
                second_finish_flag = False
                time.sleep(2)
            ...
            if first_finish_flag and not self.is_after_wednesday_4am():
                break
            if not first_finish_flag and self.auto.find_element(
                "准备作战",
                "text",
                crop=(1675 / 1920, 988 / 1080, 1833 / 1920, 1051 / 1080),
                is_log=self.is_log,
            ):
                if not self.auto.find_element(
                    "快速测评",
                    "text",
                    crop=(1236 / 1920, 943 / 1080, 1552 / 1920, 1),
                    is_log=self.is_log,
                ):
            ...
            if first_finish_flag and self.is_after_wednesday_4am() and not enter_second:
                if self.auto.click_element(
                    "稳定值",
                    "text",
                    crop=(260 / 2560, 431 / 1440, 410 / 2560, 460 / 1440),
                    is_log=self.is_log,
            ...
            if (
                enter_second
                and first_finish_flag
                and not self.auto.find_element(
                    "快速测评",
                    "text",
                    crop=(1236 / 1920, 943 / 1080, 1552 / 1920, 1),
                    is_log=self.is_log,
            ...
            if self.auto.click_element(
                ...
                continue
            if self.auto.click_element(
                "精神", "text", crop=(0, 758 / 1080, 1, 828 / 1080), is_log=self.is_log
            ):
                continue
            if self.auto.click_element(
                "特别派遣",
                "text",
                crop=(181 / 1920, 468 / 1080, 422 / 1920, 541 / 1080),
                is_log=self.is_log,
            ):
                time.sleep(0.3)
                continue
        ...
        while True:
            ...
            if enter_flag:
                ...
                if self.auto.click_element(
                    "键领取",
                    "text",
                    crop=(2226 / 2560, 1300 / 1440, 2430 / 2560, 1362 / 1440),
                    is_log=self.is_log,
                ):
                    continue
                else:
    ...
    def is_after_wednesday_4am():
        ...
        wednesday_4am = now.replace(
            hour=4, minute=0, second=0, microsecond=0
        ) + timedelta(days=2 - current_weekday)
        # 判断当前时间是否在本周三凌晨4点之后
    ...
    def is_in_time_range():
        ...
        # 周二上午10点
        tuesday_10am = now.replace(
            hour=10, minute=0, second=0, microsecond=0
        ) + timedelta(days=1 - current_weekday)
        # 下周一凌晨4点
        next_monday_4am = now.replace(
            hour=4, minute=0, second=0, microsecond=0
        ) + timedelta(days=(0 - current_weekday) + 7)
