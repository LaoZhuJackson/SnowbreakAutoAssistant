...
class PersonModule:
    def __init__(self, auto, logger):
        ...
        self.character_list = [
            self.select_person_dic["LineEdit_c1"],
            self.select_person_dic["LineEdit_c2"],
            self.select_person_dic["LineEdit_c3"],
            self.select_person_dic["LineEdit_c4"],
        ]
    ...
    def enter_person(self):
        ...
        while True:
            ...
            if self.auto.find_element(
                "故事", "text", crop=(0, 0, 212 / 1920, 61 / 1080), is_log=self.is_log
            ):
                break
            if self.auto.click_element(
                "个人故事",
                "text",
                crop=(673 / 1920, 806 / 1080, 953 / 1920, 889 / 1080),
                is_log=self.is_log,
            ):
                time.sleep(0.3)
                continue
    ...
    def find_person_and_quick_fight(self, person_name):
        ...
        while True:
            ...
            if fight_flag and self.auto.find_element(
                "完成",
                "text",
            ...
            if finish_flag:
                if self.auto.click_element(
                    ...
                    if self.auto.find_element(
                        "故事", "text", crop=(0, 0, 212 / 1920, 61 / 1080)
                    ) and (self.power_times == 0 or self.power_times == 6):
                        break
                continue
            if self.auto.find_element(
                "快速作战",
                "text",
                crop=(856 / 1920, 229 / 1080, 1056 / 1920, 295 / 1080),
                is_log=self.is_log,
            ):
                self.auto.click_element(
                    "最大",
                    "text",
                    crop=(1225 / 1920, 683 / 1080, 1341 / 1920, 750 / 1080),
                    is_log=self.is_log,
                )
                self.auto.click_element(
                    "开始作战",
                    "text",
                    crop=(873 / 1920, 807 / 1080, 1047 / 1920, 864 / 1080),
                    is_log=self.is_log,
                )
                fight_flag = True
                time.sleep(1.5)
            ...
            if pos:
                ...
                if quick_fight_pos:
                    ...
                    self.logger.warn(
                        f"未找到对应速战，检查是否已刷取或是否解锁{person_name}的速战"
                    )
                    break
            else:
                self.scroll_page()
    ...
    def find_quick_fight(self, name_pos, person_name):
        ...
        :return: (x,y)|none
        """
        pos, min_distance = self.auto.find_target_near_source(
            "速战", name_pos, crop=(0, 868 / 1080, 1, 940 / 1080), is_log=False
        )
        if pos:
            # 适配屏幕缩放
    ...
    def use_chip(self):
        ...
        while True:
            ...
            if self.auto.find_element(
                "没有该类道具",
                "text",
                crop=(821 / 1920, 511 / 1080, 1092 / 1920, 568 / 1080),
                is_log=self.is_log,
            ):
                self.no_chip = True
                return False
            ...
            if self.auto.find_element(
                "记忆嵌片",
                "text",
                crop=(803 / 1920, 275 / 1080, 991 / 1920, 346 / 1080),
                is_log=self.is_log,
            ):
            ...
            if self.auto.find_element(
                "是否",
                "text",
                crop=(588 / 1920, 309 / 1080, 1324 / 1920, 384 / 1080),
                is_log=self.is_log,
            ):
                confirm_flag = True
            if confirm_flag:
                if self.auto.click_element(
                    "最大",
                    "text",
                    crop=(1722 / 2560, 797 / 1440, 1848 / 2560, 867 / 1440),
                    is_log=self.is_log,
                ):
                    self.auto.click_element(
                        "确定",
                        "text",
                        crop=(1353 / 1920, 729 / 1080, 1528 / 1920, 800 / 1080),
                        is_log=self.is_log,
                    )
                    self.auto.press_key("esc")
                    time.sleep(1)
            ...
            if self.auto.click_element(
                "故事",
                "text",
                crop=(110 / 1920, 17 / 1080, 212 / 1920, 61 / 1080),
                n=30,
                offset=(1400 / self.auto.scale_x, 21 / self.auto.scale_y),
                is_log=self.is_log,
            ):
                time.sleep(0.5)
                continue
    ...
    def scroll_page(self, direction: int = 1, page=1):
        ...
        self.auto.mouse_scroll(
            int(904 / self.auto.scale_x),
            int(538 / self.auto.scale_y),
            7000 * direction * page,
        )
    ...
    def update_power_times(self):
        ...
        result = self.auto.read_text_from_crop(
            crop=(1421 / 1920, 27 / 1080, 1538 / 1920, 76 / 1080)
        )
        # 取出文字送去正则匹配
