...
class FishingModule:
    ...
    def run(self):
        ...
        self.upper_yellow = np.array(
            [int(value) for value in config.LineEdit_fish_upper.value.split(",")]
        )
        self.lower_yellow = np.array(
            [int(value) for value in config.LineEdit_fish_lower.value.split(",")]
        )
        ...
        while True:
            ...
            if enter_flag:
                ...
                if self.auto.find_element(
                    "钩",
                    "text",
                    crop=(787 / 1920, 234 / 1080, 1109 / 1920, 420 / 1080),
                    threshold=0,
                    is_log=self.is_log,
                ):
                    self.auto.press_key(self.press_key)
            ...
            if self.auto.find_element(
                "使用",
                "text",
                crop=(1405 / 1920, 654 / 1080, 1503 / 1920, 747 / 1080),
                is_log=self.is_log,
            ):
    ...
    def select_lure(self):
        ...
        while True:
            ...
            if self.auto.find_element(
                lure_type_list[lure_type_index],
                "text",
            ...
            if not open_lure:
                self.auto.click_element(
                    "app/resource/images/fishing/select_lure.png",
                    "image",
    ...
    def start_fish(self):
        ...
        while True:
            self.auto.take_screenshot(
                crop=(1130 / 1920, 240 / 1080, 1500 / 1920, 570 / 1080),
                is_interval=False,
            )

            if config.ComboBox_fishing_mode.value == 0:
                # blocks_num = self.count_yellow_blocks(self.auto.current_screenshot)
                blocks_num = count_color_blocks(
                    self.auto.current_screenshot, self.lower_yellow, self.upper_yellow
                )
                if blocks_num >= 2:
    ...
    def after_fish(self):
        ...
        while True:
            ...
            if self.auto.find_element(
                "本次获得",
                "text",
                crop=(832 / 1920, 290 / 1080, 1078 / 1920, 374 / 1080),
                is_log=self.is_log,
            ):
            ...
            if self.auto.find_element(
                "鱼跑掉了",
                "text",
                crop=(858 / 1920, 151 / 1080, 1054 / 1920, 280 / 1080),
                is_log=self.is_log,
            ):
    ...
    def get_press_key(self):
        ...
        自动获取钓鱼按键
        """
        text_results = self.auto.read_text_from_crop(
            (1706 / 1920, 1024 / 1080, 1820 / 1920, 1066 / 1080),
            is_log=self.is_log,
            is_screenshot=True,
            extract=[(222, 230, 236), 128],
        )
        # 根据文本内容模糊匹配键盘按键
