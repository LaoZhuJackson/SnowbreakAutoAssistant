...
class WaterBombModule:
    ...
    def enter_and_start(self):
        ...
        while True:
            ...
            if self.auto.click_element(
                "最佳记录",
                "text",
                crop=(1279 / 1920, 619 / 1080, 1382 / 1920, 658 / 1080),
                is_log=self.is_log,
                threshold=self.template_threshold,
            ):
                continue
    ...
    def fight(self):
        ...
        while True:
            ...
            if self.auto.click_element(
                "确定",
                "text",
                crop=(1418 / 1920, 740 / 1080, 1509 / 1920, 791 / 1080),
                is_log=self.is_log,
                threshold=self.template_threshold,
            ):
                continue
            ...
            if not self.is_speed_up:
                if not self.auto.find_element(
                    "app/resource/images/water_bomb/speed.png",
                    "image",
                    ...
                    self.auto.move_click(
                        1742 / self.auto.scale_x, 80 / self.auto.scale_y, press_time=0.1
                    )
                    time.sleep(0.5)
                    continue
                else:
            ...
            # 确认加速后进入主循环
                ...
                if is_player_round:
                    if current_strategy == "":
                        ...
                        self.logger.info(
                            f"当前最佳操作为：{current_strategy}, 胜率为：{win_prob}"
                        )
                        if win_prob == 0:
                            self.restart()
                            continue
                        ...
                        if current_strategy in self.normal_items:
                            if self.use_normal_item(current_strategy):
                                ...
                                if current_strategy == "gem_of_life":
                                    ...
                                    self.is_reversal = (
                                        False if self.is_reversal else True
                                    )
    ...
    def handle_shooting(self, person):
        ...
        while True:
            ...
            if self.auto.find_element(
                "更换",
                "text",
                crop=(872 / 1920, 477 / 1080, 1120 / 1920, 550 / 1080),
                is_log=self.is_log,
                threshold=self.template_threshold,
            ):
                self.is_reversal = False
            ...
            # 射击敌人
                ...
                if self.have_extra_shoot and self.auto.find_element(
                    "追加",
                    "text",
                    crop=(872 / 1920, 477 / 1080, 1120 / 1920, 550 / 1080),
                    is_log=self.is_log,
                    ...
                    return True
                if self.auto.find_element(
                    "app/resource/images/water_bomb/gun.png",
                    "image",
                    ...
                    self.auto.move_click(
                        int(960 / self.auto.scale_x), int(540 / self.auto.scale_y)
                    )
                    continue
                if self.auto.click_element(
                    path,
                    "image",
                    crop=(1640 / 1920, 900 / 1080, 1880 / 1920, 994 / 1080),
                    is_log=self.is_log,
                    threshold=self.template_threshold,
                    match_method=cv2.TM_CCOEFF_NORMED,
                ):
        ...
        while True:
            ...
            if self.auto.ocr_result:
                for ocr_result in self.auto.ocr_result:
                    if "空" in ocr_result[0] or self.auto.find_element(
                        "app/resource/images/water_bomb/blank.png",
                        "image",
                        crop=(1470 / 2560, 1290 / 1440, 1640 / 2560, 1364 / 1440),
                        match_method=cv2.TM_CCOEFF_NORMED,
                        is_log=self.is_log,
                        ...
                        self.bullet_type = 0
                        return
                    elif "水" in ocr_result[0] or self.auto.find_element(
                        "app/resource/images/water_bomb/live.png",
                        "image",
                        crop=(1470 / 2560, 1290 / 1440, 1640 / 2560, 1364 / 1440),
                        match_method=cv2.TM_CCOEFF_NORMED,
                        is_log=self.is_log,
    ...
    def select_and_reset(self):
        ...
        while True:
            ...
            if self.auto.find_image_and_count(
                self.auto.get_crop_form_first_screenshot(
                ...
                select_flag = False
            if select_flag and not self.auto.find_element(
                "确定",
                "text",
                ...
                finish_flag = True
            if finish_flag and not self.auto.find_element(
                "查看道具",
                "text",
                crop=(113 / 1920, 984 / 1080, 220 / 1920, 1020 / 1080),
                is_log=self.is_log,
    ...
    def scroll_to_bottom(self):
        self.auto.mouse_scroll(
            int(960 / self.auto.scale_x), int(540 / self.auto.scale_y), -550
        )
    ...
    def use_normal_item(self, item_name):
        ...
        while True:
            ...
            if self.auto.find_element(
                ...
                self.sustain = True
                return False
            if appear_use_button and not self.auto.find_element(
                "使用",
                "text",
            ...
            if not appear_use_button:
                self.auto.click_element(
                    path,
                    "image",
                    crop=(145 / 1920, 758 / 1080, 665 / 1920, 852 / 1080),
                    is_log=self.is_log,
                    threshold=self.template_threshold,
                    match_method=cv2.TM_CCOEFF_NORMED,
                )
    ...
    def update_extra_status(self):
        """检查当前对面是否被铐上手铐"""
        if self.auto.find_element(
            "app/resource/images/water_bomb/is_handcuffs.png",
            "image",
            threshold=self.template_threshold,
    ...
    def update_hp_and_bullet(self):
        ...
        self.current_player_hp = self.auto.find_image_and_count(
            player_hp_screenshot,
            "app/resource/images/water_bomb/hp.png",
            threshold=self.count_threshold,
            is_log=self.is_log,
        )
        no_hp = self.auto.find_image_and_count(
            player_hp_screenshot,
            "app/resource/images/water_bomb/no_hp.png",
            threshold=self.count_threshold,
            is_log=self.is_log,
        )
        self.max_hp = self.current_player_hp + no_hp
        self.current_enemy_hp = self.auto.find_image_and_count(
            enemy_hp_screenshot,
            "app/resource/images/water_bomb/hp.png",
            threshold=self.count_threshold,
            is_log=self.is_log,
        ...
        is_num = True
        if self.auto.find_element(
            f"app/resource/images/water_bomb/q.png",
            "image",
        ...
        if is_num:
            for i in range(0, 5):
                if self.auto.find_element(
                    f"app/resource/images/water_bomb/{i}.png",
                    "image",
                    crop=(2000 / 2560, 1280 / 1440, 2137 / 2560, 1397 / 1440),
                    threshold=self.template_threshold,
                    is_log=self.is_log,
        ...
        self.logger.info(
            f"当前玩家血量：{self.current_player_hp}，对方血量：{self.current_enemy_hp}"
        )
        self.logger.info(
            f"当前水弹数量：{self.remaining_live_bullet}，空弹数量：{self.remaining_blank_bullet}"
        )
        # time.sleep(5)
    ...
    def get_items(self, crop):
        ...
        for path in path_list:
            ...
            for i in range(
                self.auto.find_image_and_count(
                    crop_screenshot,
                    path,
                    threshold=self.count_threshold,
                    is_log=self.is_log,
