...
class Additional(QFrame, Ui_additional_features, BaseInterface):
        ...
        for widget in self.findChildren(QWidget):
            ...
            if config_item:
                if isinstance(widget, CheckBox):
                    ...
                    text_name = (
                        "BodyLabel_"
                        + widget.objectName().split("_")[1]
                        + "_"
                        + widget.objectName().split("_")[2]
                    )
    ...
    def save_changed(self, widget):
        if isinstance(widget, SpinBox):
            ...
            if widget.objectName().split("_")[1] == "fish":
                ...
                if widget.objectName().split("_")[2] == "key":
                    config.set(
                        getattr(config, widget.objectName(), None), widget.text()
                    )
                else:
                    text = widget.text()
    ...
    def set_simple_card_enable(self, simple_card, enable: bool):
        ...
        for child in children:
            if (
                isinstance(child, CheckBox)
                or isinstance(child, LineEdit)
                or isinstance(child, SpinBox)
            ):
